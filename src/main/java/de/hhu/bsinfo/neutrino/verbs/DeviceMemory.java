...
public class DeviceMemory extends Struct implements AutoCloseable {
    ...
    public boolean copyFromDeviceMemory(long sourceOffset, LocalBuffer targetBuffer, long targetOffset, long length) {
        ...
        if(isError) {
            LOGGER.error("Copying from deviceAttributes memory to local buffer failed with error [{}]", result.getStatus());
        }
    ...
    public boolean copyToDeviceMemory(LocalBuffer sourceBuffer, long sourceOffset, long targetOffset, long length) {
        ...
        boolean isError = result.isError();
        if(isError) {
            LOGGER.error("Copying from local buffer to deviceAttributes memory failed with error [{}]", result.getStatus());
        }
    ...
    public void close() {
        ...
        boolean isError = result.isError();
        if(isError) {
            LOGGER.error("Freeing deviceAttributes memory failed with error [{}]", result.getStatus());
        }
    ...
    public static final class AllocationAttributes extends Struct {
        ...
        public AllocationAttributes(Consumer<AllocationAttributes> configurator) {
